		...
		jQuery.ajax({
			...
			success: function(data) {
				...
				if (parent != '0') {
					...
					if ( O_Connor.formpostion == 'bottom' ) {
						jQuery('#respond').before('<ol class="commentlist">' + data + '</ol>');
					} else {
						jQuery('#respond').after('<ol class="commentlist">' + data + '</ol>');
					}
					
				} else {
					if ( O_Connor.order == 'asc' ) {
						jQuery('.commentlist').append(data);// your comments wrapper
					} else {
						jQuery('.commentlist').prepend(data);// your comments wrapper
					}
